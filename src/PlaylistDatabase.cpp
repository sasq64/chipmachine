...
PlaylistDatabase::PlaylistDatabase() : db(File::getCacheDir() / "play.db") {
	...
	db.exec("CREATE TABLE IF NOT EXISTS song (playlist INT, title STRING, game STRING, composer "
	        "STRING, format STRING, path STRING)");
	...
	*/
	...
	for(auto &pl : playlists) {
		auto q = db.query<string, string, string, string, string>(
		    "SELECT path,game,title,composer,format FROM song WHERE playlist=?", id);
		while(q.step()) {
			SongInfo song;
			// int cid;
			// string path;
...
void PlaylistDatabase::addToPlaylist(const std::string &name, const SongInfo &song) {
	...
	for(auto &p : playlists) {
		if(p.name == name) {
			...
			db.exec("INSERT INTO song (playlist, title, game, composer, format, path) VALUES (?, "
			        "?, ?, ?, ?, ?)",
			        id, song.title, song.game, song.composer, song.format, song.path);
...
/*Playlist */ void PlaylistDatabase::getPlaylist(const std::string &name,
                                                 std::function<void(const Playlist &)> cb) {
	for(auto &p : playlists) {
		if(p.name == name) {
#ifdef USE_REMOTELISTS
			if(p.isRemote) {
				RemoteLists::getInstance().getList(
				    name, [=](const string &name, const vector<string> &list) {
					    ...
					    for(const auto &l : list) {
						    ...
						    pl->songs.push_back(song);
						    // auto collection =
						    // MusicDatabase::getInstance().getCollection(parts[1]);
...
int PlaylistDatabase::search(const string &query, vector<string> &result) {
	...
	if(query == "") {
		...
		// transform(playlists.begin(), playlists.end(), result.begin(), [](const Playlist &pl) ->
		// string { return pl.name; });
	} else {
		for(const auto &p : playlists) {
