...
int main(int argc, char* argv[]) {
	...
	while(true) {
		...
		if(cmd == "find") {
			...
			string qs = "SELECT path,title,composer FROM song WHERE ";
			...
			//LOGW(qs);
			auto q = db.query<string, string, string>(qs, parts);
/*
			auto q = parts.size() > 2 ? 
...
*/
			...
			//modlandPath = "ftp://ftp.modland.com/pub/modules/";
