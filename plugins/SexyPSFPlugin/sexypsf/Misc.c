...
static void FreeTags(PSFTAG *tags) {
	while(tags) {
		...
		//__android_log_print(ANDROID_LOG_VERBOSE, "SexyPSF", "tag %p %p %p", tags, tags->key, tags->value);
...
static PSFINFO *LoadPSF(char *path, int level, int type) // Type==1 for just info load.
{
	...
	//__android_log_print(ANDROID_LOG_VERBOSE, "SexyPSF", "Header found");
	...
	if(type)
		...
		if(level) {
			//__android_log_print(ANDROID_LOG_VERBOSE, "SexyPSF", "LoadPSXMem");
		...
		if(fread(tagdata, 1, 5, fp) == 5) {
			if(!memcmp(tagdata, "[TAG]", 5)) {
				...
				while(fgets(linebuf, 1024, fp) > 0) {
					...
					if(!level) {
						static char *yoinks[8] = { "title", "artist", "game", "year", "genre", "copyright", "psfby", "comment" };
						char **yoinks2[8] = { &psfi->title, &psfi->artist, &psfi->game, &psfi->year, &psfi->genre, &psfi->copyright, &psfi->psfby,
								&psfi->comment };
						for(x = 0; x < 8; x++)
...
PSFINFO *sexy_load(char *path) {
	...
	//__android_log_print(ANDROID_LOG_VERBOSE, "SexyPSF", "Loading");
