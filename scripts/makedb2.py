...
def scanDir(path, depth) :
	...
	for f in files :
		if f.lower().endswith('.sid') :
			...
			dbc.execute('INSERT OR IGNORE INTO composers VALUES (null, ?)', (composer,))
			composer_id = dbc.lastrowid
			dbc.execute('insert into songs values (null, ?,?,?,?)', (str(f), title, composer_id, 'COPYRIGHT='+copyright))
	db.commit()
...
def main(argv) :
	...
	dbc = db.cursor()
	try :
		dbc.execute('CREATE TABLE songs (_id INTEGER PRIMARY KEY AUTOINCREMENT, path TEXT, title TEXT, composer_id INTEGER, metadata TEXT, FOREIGN KEY(composer_id) REFERENCES composer(_id))')
		dbc.execute('CREATE TABLE composers (_id INTEGER PRIMARY KEY AUTOINCREMENT, composer TEXT, UNIQUE(composer))'  )
